...
if(status==1){
	...
	$.ajax({
		url: "${rc.contextPath}/admin/am/res/editCommit",
		type: "POST",
		async: false,
		dataType: "json",
		traditional: true,
...
<div id="attrvals">
	...
	<#if attrval??>
		<#list attrval as item>
			<div>
				<input type="text" name="attrval" class='inputStyle' value="${item.ATTR_VAL_NAME}"/>
				<input type="text" name="attrvalDes" value="${item.VALUES1}" class='cssdes inputStyle'/>
				<input type="button" value="删除" onclick="delAttrval(this)" class="mediumBtn"/>
			</div>
		</#list>
	</#if>
